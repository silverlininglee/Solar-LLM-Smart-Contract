...
- 이 프로젝트는 Upstage의 Product Usecase인 부동산 문서 정보 추출 서비스 **'Boomerang'** 을 벤치마킹하여, PM의 관점에서 비즈니스 가치를 더해 고도화하였습니다.
- Document(계약서, 등기 등)의 데이터를 추출하는 기존 기능에 Solar LLM의 추론 능력을 결합하여 사용자에게 **실질적인 의사결정 가이드**를 제공하는 전세사기 진단 엔진을 설계했습니다.
...
### 1.1. 배경 및 문제 정의
- **기존 OCR/추출 모델의 한계**: Upstage `information-extract` API(부메랑 기반)는 계약서와 등본 내 핵심 데이터를 정확히 뽑아내지만, 부동산 법률 지식이 부족한 일반 사용자는 이 데이터가 의미하는 것이 무엇인지, 그래서 이 계약이 위험한지 아닌지 판단하기 어려운 한계가 있습니다.
  
- **정보의 비대칭성**: 임대인과 임차인 사이의 정보 격차로 인해, 임차인은 자신에게 불리한 독소 조항을 인지하지 못한 채 계약을 체결하여 보증금 미반환 리스크에 노출됩니다.


### 1.2. 해결 방법
- **추출에서 추론까지**: 부메랑의 데이터 추출 결과를 Solar LLM의 추론 엔진에 통과시켜 **'데이터 나열'에서 '법률적 인사이트'** 로 가치를 전환합니다.
